...
void PlannerData::Initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p)
{
  ...
  selected_viewpoint_vis_cloud_ = 
      std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "selected_viewpoint_vis_cloud", kWorldFrameID);
...
void SensorCoveragePlanner3D::ResetWaypointCallback(const std_msgs::Empty::ConstPtr& empty_msg){
  ...
  waypoint.pose.position.x = pd_.robot_position_.x;
  waypoint.pose.position.y = pd_.robot_position_.y;
...
void SensorCoveragePlanner3D::execute(const ros::TimerEvent&)
{
  ...
  if (keypose_cloud_update_)
  {
    ...
    // ROS_INFO("Checking exploration finish conditions:");
    // ROS_INFO("IsReturningHome: %d", pd_.grid_world_->IsReturningHome());
    // ROS_INFO("IsLocalCoverageComplete: %d", pd_.local_coverage_planner_->IsLocalCoverageComplete());
    // ROS_INFO("Time since start: %.2f", (ros::Time::now() - start_time_).toSec());
