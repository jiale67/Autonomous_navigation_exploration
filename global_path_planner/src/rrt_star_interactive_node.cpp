...
std::vector<double> exactGoalPoint {0, 0, 1.0};  // 保存精确的点击终点
void clickedPointCB(const geometry_msgs::PoseStamped::ConstPtr& cp){
    exactGoalPoint[0] = cp->pose.position.x;  // 保存精确坐标
    exactGoalPoint[1] = cp->pose.position.y;
    exactGoalPoint[2] = cp->pose.position.z;
    
    goalPoint[0] = cp->pose.position.x;
    goalPoint[1] = cp->pose.position.y;
    goalPoint[2] = 1.0; // set height to be 1.0 m for planning
    newGoalMsg = true;
}
...
int main(int argc, char** argv){
    ...
    while (ros::ok()){
        ...
        // 设置精确的终点用于可视化（确保 /global_planned_path 的最后一个航点是精确点击坐标）
        rrtStarPlanner.setExactGoalForVisualization(exactGoalPoint);
