...
namespace globalPlanner{
	...
	public:
		...
		// 设置精确的终点用于可视化（确保最后一个航点是精确的点击终点）
		void setExactGoalForVisualization(const std::vector<double>& exactGoal);
	...
	void rrtOctomap<N>::startVisModule(){
		...
		if (this->visPath_){
			this->pathVisPub_ = this->nh_.advertise<visualization_msgs::MarkerArray>("/global_planned_path", 10);
	...
	template <std::size_t N>
	void rrtOctomap<N>::setExactGoalForVisualization(const std::vector<double>& exactGoal){
		// 修改最后一个航点的坐标为精确的终点
		if (!this->pathVisVec_.empty() && exactGoal.size() >= N) {
			auto& lastMarker = this->pathVisVec_.back();
			if (lastMarker.type == visualization_msgs::Marker::SPHERE) {
				lastMarker.pose.position.x = exactGoal[0];
				lastMarker.pose.position.y = exactGoal[1];
				if (N >= 3 && exactGoal.size() >= 3) {
					lastMarker.pose.position.z = exactGoal[2];
				}
				cout << "[RRTPlanner]: Set exact goal for visualization (" 
				     << exactGoal[0] << ", " << exactGoal[1] << ", " << exactGoal[2] << ")" << endl;
			}
		}
	}
