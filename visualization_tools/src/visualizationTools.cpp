...
double exploredAreaPublishRate = 0.2;  // 探索区域点云发布频率 (Hz)
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  ...
  // 移除 systemInited 检查，允许在机器人移动前就累积点云
  // 这样启动后就能立即累积雷达扫描数据
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("exploredAreaPublishRate", exploredAreaPublishRate);
  ...
  // 初始化探索区域发布计数器
  int exploredAreaPublishCount = 0;
  // 根据设置的发布频率计算间隔次数
  // exploredAreaPublishRate (Hz) -> 发布间隔 = 1/rate 秒 -> 循环次数 = (1/rate) * 100
  int exploredAreaPublishInterval = static_cast<int>(100.0 / exploredAreaPublishRate);
  
  ROS_INFO("[VisualizationTools] Explored areas publish rate: %.2f Hz (every %.1f seconds)", 
           exploredAreaPublishRate, 1.0 / exploredAreaPublishRate);
  ...
  while (status) {
    ...
    // 定期发布探索区域点云（即使为空也发布）
    exploredAreaPublishCount++;
    if (exploredAreaPublishCount >= exploredAreaPublishInterval) {
      sensor_msgs::PointCloud2 exploredArea2;
      pcl::toROSMsg(*exploredAreaCloud, exploredArea2);
      exploredArea2.header.stamp = ros::Time::now();
      exploredArea2.header.frame_id = "map";
      pubExploredArea.publish(exploredArea2);
      
      exploredAreaPublishCount = 0;
    }
